...
def oracle_preflight(args, output, environment, deadline):
    ...
    for directory in sorted((output / "oracle").glob("case.*")):
        ...
        require(native_verdict == verdict == "UNSATISFIABLE", f"unexpected baseline verdict: {name}")
...
def validate_sample(case, arm, record):
    ...
    if arm != "native":
        ...
        require(normalized_ems(data) == case["reference"], "AOT output differs from preflight VM reference")
    else:
...
def arguments(argv):
    ...
    require(0 < args.timeout <= 86400 and 0 < args.budget_seconds <= 604800, "invalid timeout or total budget")
    seen = {"3x3", "4x4"}
...
def main(argv=None):
    ...
    try:
        ...
        binaries = {label: {"path": str(path), "sha256": sha256(path)}
                    for label, path in {**commands, "vm": args.vm}.items()}
        ...
        if args.candidate_binary:
            candidate = {"label": args.candidate_label, "source_path": str(args.candidate_source),
                         ...
                         "scope": "timing experiment; every measured stdout matches the preflight VM reference; no broader correctness claim"}
        ...
        manifest = {"schema": 1, "status": "incomplete", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                    "n": REPETITIONS, "timing_scope": TIMING, "aot_policy": EMS_POLICY, "native_policy": NATIVE_POLICY,
                    "warmup": "one unmeasured native/VM/baseline-AOT correctness preflight per case; optional candidate gets one unmeasured validated run per case",
        ...
        rows = measure(cases, commands, output, environment, args.timeout, deadline, args.candidate_label)
        ...
        with (output / "summary.tsv").open("w") as table:
            table.write("case\tarm\tpolicy\tn\tmedian_wall_s\tmin_wall_s\tmax_wall_s\tconflicts\tdescriptive_conflicts_per_wall_second\n")
            for row in rows:
                for arm in commands:
                    value = row[arm]
                    table.write(f"{row['case']}\t{arm}\t{row[arm + '_policy']}\t{REPETITIONS}\t"
